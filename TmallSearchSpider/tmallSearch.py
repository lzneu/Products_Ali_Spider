...
# *********************************************************************************
#             第一步：访问指定商品页面 https://item.jd.com/1218759.html#comments-list
#                  获取基本信息 VisitGoodsPage()
#
# *********************************************************************************

# def dealWithElements(mongo_collection,shopID_list,name_list,price_list,sellNum_list):
#
#   # for i in range(len(shopID_list)):
#   #               # print(gl_item_list[i].get_attribute('data-sku'))
#   #               # https://item.jd.com/1039795.htmldata-nick
#   #               # if 'https://item.jd.com/' in item_list[i].get_attribute('comproid'):
#   #               # print(shop_list[i].get_attribute('data-nick'))
#   #               print(shopID_list[i].get_attribute('data-userid'))
#   #               print(shopID_list[i].text)
#   #               # item_id =item_list[i].get_attribute('href').split('https://item.jd.com/')[1].split('.')[0]
#   #               # print(item_id)
#   #               # print(name_list[i].text)
#   #               print(sellNum_list[i].text)
#   #
#   #               print(price_list[i].text)
#   #               print(name_list[i].text,name_list[i].get_attribute('data-nid'))
#   #               # print(name_list[i])
#   #               # print(commit_list[i].text)
...
#
#             # print(shopID_list[i].get_attribute('data-userid'))
#             seller_id =shopID_list[i].get_attribute('data-userid')
#             print(seller_id)
#             seller_name =shopID_list[i].text
#
#
#             item_id =name_list[i].get_attribute('data-nid')
#             print(item_id)
#
#             item_title =name_list[i].text
#             print(item_title)
#             item_price =price_list[i].text
#             print(item_price)
#
#             sell_num =sellNum_list[i].text
#
#
#
#             item_dict ={}
#             item_dict['product_id'] = int(item_id)
#             item_dict['product_title'] = item_title
#             item_dict['seller_name'] = seller_name
#             item_dict['seller_id'] = int(seller_id)
#             item_dict['price'] = item_price
#             item_dict['sell_num'] = sell_num
#             item_dict['catagory'] = '洗衣片'
#             insertToMongo(mongo_collection, item_dict)
#
#              # item_dict['price_list'] =int(item_id)
...
if __name__ == '__main__':
    ...
    if False:
        for key in key_list:
            # 0是url， 1是collection名 ,key 是类别

            url = urls_collections_config[key][0]
            category = key
            category_id = urls_collections_config[key][2]
            brand_collection_name = urls_collections_config[key][1] + '_brands'
            # 获取品牌信息同时更新产品品牌collection
            brand_list = setBrandList(category, category_id, driver, url, mongo_conn, brand_collection_name)
            for brand in brand_list:
                Logger.info(brand)

    for key in key_list:
        collection_name = urls_collections_config[key][1]
        collection = dataBase[urls_collections_config[key][1]]  # collection
        Logger.info(collection_name)


        Logger.info('###################产品类别：' + key + '#############################')
        Logger.info('''
                           ############################################################
                           |                      进入各个品牌                           |
                           ############################################################
                       ''')
        brand_list = getBrandList(collection_name, mongo_conn)
        for brand in brand_list:
            Logger.info(brand['original_name'])
